...
from app.schemas.interaction import LeadScoreCreate, InteractionLogCreate, CallInteractionCreate, MessageInteractionCreate
...
class LeadScoringService:
    ...
    def get_lead_rankings(self, customer_id: int) -> List[Dict]:
        """Get ranked list of leads for a customer."""
        ...
        return sorted(lead_scores, key=lambda x: x["score"], reverse=True)

    def get_high_value_leads(self, customer_id: int, threshold: float = 70.0) -> List[Lead]:
        """Get leads with scores above the threshold."""
        leads = self.db.query(Lead).filter(
            Lead.customer_id == customer_id
        ).all()
