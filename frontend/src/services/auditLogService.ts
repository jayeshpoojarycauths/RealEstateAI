...
export const auditLogService = {
  ...
  async getById(id: string): Promise<AuditLog> {
    return apiClient.get<AuditLog>(
      `${API_CONFIG.endpoints.auditLogs.detail}/${id}`,
    );
  },
  ...
  async getByUser(
    ...
    return apiClient.get<PaginatedResponse<AuditLog>>(
      `${API_CONFIG.endpoints.auditLogs.byUser}/${userId}`,
      {
        params: filters,
      },
    );
  ...
  async getByResource(
    ...
    return apiClient.get<PaginatedResponse<AuditLog>>(
      `${API_CONFIG.endpoints.auditLogs.byResource}/${resourceType}/${resourceId}`,
      {
        params: filters,
      },
    );
  ...
  async export(filters?: AuditLogFilters): Promise<Blob> {
    const response = await apiClient.get<Blob>(
      API_CONFIG.endpoints.auditLogs.export,
      {
        params: filters,
        responseType: "blob",
      },
